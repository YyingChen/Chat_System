...
static void* run_output_friend(void*arg)
{
      ...
      for(;;)
      {
        ...
        std::string str;
        _cli->recvData(str);
        recvData.stringToValue(str);//反序列化
        ...
        tips+=recvData.msg;
        getmaxyx(win.output,h,w);        
        win.putStringToWin(win.output,point++,1,tips);//行变
        ...
        {
        win.putStringToWin(win.friend_list,fpoint++,1,*iter);//行变
...
static void* run_input(void* arg)
{
  ...
  for(;;)
  {
      ...
      sendData.valueToString(sendString);//序列化
...
int main(int argc,char* argv[])//./client 126.1.1.1 8080 nick_name school
{
    if(argc!=5)
    {
        usage(argv[0]);
